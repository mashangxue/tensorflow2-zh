...
### 1.1. 导入 TensorFlow 和其他库 Import TensorFlow and other libraries
...
### 1.2. 下载莎士比亚数据集 Download the Shakespeare dataset
...
## 2. 处理文本 Process the text

### 2.1. 向量化文本 Vectorize the text
...
### 2.3. 创建训练样本和目标 Create training examples and targets
...
### 2.4. 使用 tf.data 创建批次文本并重排这些批次 Create training batches
...
### 5.1. 添加优化器和损失函数 Attach an optimizer, and a loss function
...
### 5.2. 配置检查点 Configure checkpoints
...
## 6. 生成文本 Generate text

### 6.1. 加载最新的检查点 Restore the latest checkpoint
...
## 7. 高级：自定义训练 Advanced: Customized Training
